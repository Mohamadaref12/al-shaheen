...
<body>
    <div class="brand">Al Shaheen Media — {{ $locale === 'ar' ? 'الأخبار' : 'News' }}</div>

    @if ($news->category)
        <div class="category">{{ $news->category->name }}</div>
    @endif
    ...
    <div class="meta">
        @if ($news->author)
            <span>{{ $locale === 'ar' ? 'الكاتب:' : 'Author:' }} {{ $news->author->name }}</span>
        @endif

        @if ($news->published_at)
            <span>{{ $locale === 'ar' ? 'تاريخ النشر:' : 'Published:' }} {{ $news->published_at->format('Y-m-d') }}</span>
        @endif

        @if ($news->read_time)
            <span>{{ $locale === 'ar' ? 'وقت القراءة:' : 'Read time:' }} {{ $news->read_time }} {{ $locale === 'ar' ? 'دقيقة' : 'min' }}</span>
        @endif
    </div>
